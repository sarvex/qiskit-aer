...
from .pulse_sim_independent import (
    simulate_1q_model,
...
class TestPulseSimulator(common.QiskitAerTestCase):
    ...
    def test_arbitrary_constant_drive(self):
        ...
        omega_0 = 1.0
        omega_d_vals = [omega_0 + 1.0, omega_0 + 0.02, omega_0 + 0.005]
        ...
        for i in range(num_tests):
            with self.subTest(i=i):
                ...
                indep_yf = simulate_1q_model(
                    y0, omega_0, r_vals[i], np.array([omega_d_vals[i]]), samples, 1.0
                )
                ...
                rwa_ham = (
                    ...
                    * (
                        detuning * self.Z / 2
                        + r_vals[i] * np.array([[0, amp.conj()], [amp, 0.0]]) / 4
                    )
                )
    ...
    def test_2Q_interaction(self):
        ...
        y0 = np.kron(np.array([1.0, 0.0]), np.array([1.0, 0.0]))
    ...
    def test_2Q_exchange(self):
        ...
        yf = simulate_2q_exchange_model(y0, q_freqs, r, j, q_freqs, samples, 1.0)
    ...
    def test_delay_instruction(self):
        ...
        sched += Play(Waveform([1.0]), DriveChannel(0))
    ...
    def test_shift_phase(self):
        ...
        # run independent simulation
        samples = np.array(
            [[amp1], [amp2 * np.exp(1j * phi1)], [amp3 * np.exp(1j * (phi1 + phi2))]]
        )
        indep_yf = simulate_1q_model(y0, omega_0, r, np.array([omega_0]), samples, 1.0)
    ...
    def test_frequency_error(self):
        ...
        drive_pulse = Waveform(1.0 * np.ones(total_samples))
        schedule = Schedule()
        schedule |= Play(drive_pulse, DriveChannel(0))
    ...
    def test_schedule_freqs(self):
        ...
        pulse_sim.set_options(shots=128)
